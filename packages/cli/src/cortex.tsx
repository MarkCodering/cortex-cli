...
function getNodeMemoryArgs(config: Config): string[] {
  ...
  if (process.env['CORTEX_CLI_NO_RELAUNCH']) {
    return [];
  }
...
async function relaunchWithAdditionalArgs(additionalArgs: string[]) {
  ...
  const newEnv = { ...process.env, CORTEX_CLI_NO_RELAUNCH: 'true' };
...
export async function main() {
  ...
  if (!input) {
    console.error(
      `No input provided via stdin. Input can be provided by piping data into cortex or using the --prompt option.`,
    );
    process.exit(1);
  }
...
function setWindowTitle(title: string, settings: LoadedSettings) {
  if (!settings.merged.ui?.hideWindowTitle) {
    const windowTitle = (
      process.env['CLI_TITLE'] || `Cortex - ${title}`
    ).replace(
